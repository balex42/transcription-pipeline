...
`ghcr.io/balex42/transcription-pipeline:sha-21bdf50`; `faster_whisper_image` defaults to the
dedicated `ghcr.io/balex42/transcription-pipeline-faster-whisper:sha-21bdf50` repository and
`canary_image` to the dedicated `ghcr.io/balex42/transcription-pipeline-canary:sha-21bdf50` image.

The WorkflowTemplate and all six Python runtime image parameters form one compatible release pair.
`sha-21bdf50` must be published by the container workflow before applying this template. If it is not
