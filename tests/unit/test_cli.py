...
def test_prefetch_cohere_includes_the_forced_aligner(monkeypatch: object) -> None:
    calls: list[tuple[str, str, str | None]] = []
    ...
    assert cli.main(["prefetch-models", "--asr", "cohere"]) == 0
    assert calls == [
        ("CohereLabs/cohere-transcribe-03-2026", DEFAULT_PYANNOTE_MODEL, DEFAULT_QWEN_ALIGNER_MODEL)
    ]
...
def test_compare_defaults_to_all_production_backends() -> None:
    ...
    assert args.models == "parakeet,qwen,nemotron,voxtral,cohere"
